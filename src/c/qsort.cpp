...
struct Comparator {
    ...
    Comparator(Compare* func) : comp_func(func), comp_type(CompType::COMPARE), arg(nullptr) {}

    Comparator(CompareWithState* func, void* arg_val)
        : comp_func_r(func), comp_type(CompType::COMPARE_WITH_STATE), arg(arg_val) {}
...
#endif
    int
    comp_vals(const void* a, const void* b) const {
...
class Array {
  ...
  public:
    ...
    Array make_array(size_t i, size_t s) const { return Array(get(i), s, elem_size, compare); }
};
...
static size_t partition(const Array& array) {
    ...
    while (true) {
        ...
        while ((compare_i = array.elem_compare(i, pivot)) < 0) ++i;
        while ((compare_j = array.elem_compare(j, pivot)) > 0) --j;
...
extern "C" void qsort(void* array, size_t array_size, size_t elem_size,
                      int (*compare)(const void*, const void*)) {
    if (array == nullptr || array_size == 0 || elem_size == 0) return;
    Comparator c(compare);
    quicksort(Array(reinterpret_cast<uint8_t*>(array), array_size, elem_size, c));
}
