...
#include "src/c/string/strtonumresult.hpp"
...
static inline int b36_char_to_int(char input) {
    if (isdigit(input)) return input - '0';
    if (isalpha(input)) return (input | 32) + 10 - 'a';
    return 0;
}
...
template <class T>
static inline StrToNumResult<T> strtointeger(const char* __restrict src, int base) {
    ...
    constexpr bool IS_UNSIGNED = (std::numeric_limits<T>::min() == 0);
    const bool     is_positive = (result_sign == '+');
    unsigned long long constexpr NEGATIVE_MAX =
        !IS_UNSIGNED ? static_cast<unsigned long long>(std::numeric_limits<T>::max()) + 1
                     : std::numeric_limits<T>::max();
    unsigned long long const abs_max = (is_positive ? std::numeric_limits<T>::max() : NEGATIVE_MAX);
    unsigned long long const abs_max_div_by_base = abs_max / base;
    while (isalnum(*src)) {
        int cur_digit = b36_char_to_int(*src);
        if (cur_digit >= base) break;
    ...
    if (error_val == ERANGE) {
        if (is_positive || IS_UNSIGNED)
            return {::std::numeric_limits<T>::max(), str_len, error_val};
        else  // T is signed and there is a negative overflow
            return {std::numeric_limits<T>::min(), str_len, error_val};
    }

    return {is_positive ? static_cast<T>(result)
                        : static_cast<T>(-static_cast<std::make_unsigned_t<T>>(result)),
            str_len, error_val};
}
