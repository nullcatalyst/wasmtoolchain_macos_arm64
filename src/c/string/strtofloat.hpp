...
#include "src/c/string/fpbits.hpp"
#include "src/c/string/highprecisiondecimal.hpp"
...
static inline uint64_t low64(const __uint128_t& num) {
    return static_cast<uint64_t>(num & 0xffffffffffffffff);
}

static inline uint64_t high64(const __uint128_t& num) { return static_cast<uint64_t>(num >> 64); }

template <class T>
static inline void set_implicit_bit(fputil::FPBits<T>&) {
    return;
}

#if defined(LIBC_LONG_DOUBLE_IS_X86_FLOAT80)
template <>
static inline void set_implicit_bit<long double>(fputil::FPBits<long double>& result) {
...
template <class T>
static inline std::optional<ExpandedFloat<T>> eisel_lemire(
    ExpandedFloat<T> init_num, RoundDirection round = RoundDirection::Nearest) {
    ...
    // Exp10 Range
    if (exp10 < DETAILED_POWERS_OF_TEN_MIN_EXP_10 || exp10 > DETAILED_POWERS_OF_TEN_MAX_EXP_10) {
        return std::nullopt;
    }

    // Normalization
    uint32_t clz = std::countl_zero<StorageType>(mantissa);
    mantissa <<= clz;

    int32_t exp2 = exp10_to_exp2(exp10) + FPBits::STORAGE_LEN + FPBits::EXP_BIAS - clz;

    // Multiplication
    const uint64_t* power_of_ten =
        DETAILED_POWERS_OF_TEN[exp10 - DETAILED_POWERS_OF_TEN_MIN_EXP_10];

    __uint128_t first_approx =
        static_cast<__uint128_t>(mantissa) * static_cast<__uint128_t>(power_of_ten[1]);
    ...
    if ((high64(first_approx) & halfway_constant) == halfway_constant &&
        ...
        __uint128_t low_bits =
            static_cast<__uint128_t>(mantissa) * static_cast<__uint128_t>(power_of_ten[0]);
        __uint128_t second_approx = first_approx + static_cast<__uint128_t>(high64(low_bits));

        if ((high64(second_approx) & halfway_constant) == halfway_constant &&
            low64(second_approx) + 1 == 0 && low64(low_bits) + mantissa < mantissa) {
            return std::nullopt;
        }
    ...
    StorageType msb = static_cast<StorageType>(high64(final_approx) >> (FPBits::STORAGE_LEN - 1));
    StorageType final_mantissa = static_cast<StorageType>(
        high64(final_approx) >> (msb + FPBits::STORAGE_LEN - (FPBits::FRACTION_LEN + 3)));
    exp2 -= static_cast<uint32_t>(1 ^ msb);  // same as !msb

    if (round == RoundDirection::Nearest) {
        // Half-way ambiguity
        if (low64(final_approx) == 0 && (high64(final_approx) & halfway_constant) == 0 &&
            (final_mantissa & 3) == 1) {
            return std::nullopt;
        }
...
template <>
static inline std::optional<ExpandedFloat<long double>> eisel_lemire<long double>(
    ExpandedFloat<long double> init_num, RoundDirection round) {
    ...
    __uint128_t approx_upper =
        static_cast<__uint128_t>(high64(mantissa)) * static_cast<__uint128_t>(power_of_ten[1]);

    __uint128_t approx_middle_a =
        static_cast<__uint128_t>(high64(mantissa)) * static_cast<__uint128_t>(power_of_ten[0]);
    __uint128_t approx_middle_b =
        static_cast<__uint128_t>(low64(mantissa)) * static_cast<__uint128_t>(power_of_ten[1]);

    __uint128_t approx_middle = approx_middle_a + approx_middle_b;

    // Handle overflow in the middle
    approx_upper += (approx_middle < approx_middle_a) ? __uint128_t(1) << 64 : 0;

    __uint128_t approx_lower =
        static_cast<__uint128_t>(low64(mantissa)) * static_cast<__uint128_t>(power_of_ten[0]);

    __uint128_t final_approx_lower =
        approx_lower + (static_cast<__uint128_t>(low64(approx_middle)) << 64);
    __uint128_t final_approx_upper =
        approx_upper + high64(approx_middle) + (final_approx_lower < approx_lower ? 1 : 0);
    ...
    constexpr __uint128_t HALFWAY_CONSTANT =
        (__uint128_t(1) << (FPBits::STORAGE_LEN - (FPBits::FRACTION_LEN + 3))) - 1;
    ...
    uint32_t    msb = static_cast<uint32_t>(final_approx_upper >> (FPBits::STORAGE_LEN - 1));
    StorageType final_mantissa =
        final_approx_upper >> (msb + FPBits::STORAGE_LEN - (FPBits::FRACTION_LEN + 3));
    exp2 -= static_cast<uint32_t>(1 ^ msb);  // same as !msb
...
constexpr int32_t NUM_POWERS_OF_TWO = sizeof(POWERS_OF_TWO) / sizeof(POWERS_OF_TWO[0]);
...
template <class T>
static inline FloatConvertReturn<T> simple_decimal_conversion(
    const char* __restrict numStart, RoundDirection round = RoundDirection::Nearest) {
    using FPBits      = typename fputil::FPBits<T>;
    using StorageType = typename FPBits::StorageType;
...
template <>
class ClingerConsts<double> {
  public:
    static constexpr double  POWERS_OF_TEN_ARRAY[] = {1e0,  1e1,  1e2,  1e3,  1e4,  1e5,  1e6,  1e7,
...
#if defined(LIBC_LONG_DOUBLE_IS_FLOAT64)
template <>
class ClingerConsts<long double> {
  public:
    static constexpr long double POWERS_OF_TEN_ARRAY[] = {
        1e0,  1e1,  1e2,  1e3,  1e4,  1e5,  1e6,  1e7,  1e8,  1e9,  1e10, 1e11,
        1e12, 1e13, 1e14, 1e15, 1e16, 1e17, 1e18, 1e19, 1e20, 1e21, 1e22};
    static constexpr int32_t     EXACT_POWERS_OF_TEN = ClingerConsts<double>::EXACT_POWERS_OF_TEN;
    static constexpr int32_t     DIGITS_IN_MANTISSA  = ClingerConsts<double>::DIGITS_IN_MANTISSA;
    static constexpr long double MAX_EXACT_INT       = ClingerConsts<double>::MAX_EXACT_INT;
};
...
class ClingerConsts<long double> {
  public:
    ...
    static constexpr long double MAX_EXACT_INT       = 10384593717069655257060992658440191.0L;
};
#endif
...
template <class T>
static inline std::optional<ExpandedFloat<T>> clinger_fast_path(
    ExpandedFloat<T> init_num, RoundDirection round = RoundDirection::Nearest) {
    using FPBits      = typename fputil::FPBits<T>;
    using StorageType = typename FPBits::StorageType;
    ...
    FPBits result;
    T      float_mantissa;
    if constexpr (std::is_same_v<StorageType, std::UInt<128>>) {
        float_mantissa = static_cast<T>(
            fputil::DyadicFloat<128>(Sign::POS, 0,
    ...
    if (exp10 > 0) {
        if (exp10 > ClingerConsts<T>::EXACT_POWERS_OF_TEN + ClingerConsts<T>::DIGITS_IN_MANTISSA) {
            return std::nullopt;
        }
        if (exp10 > ClingerConsts<T>::EXACT_POWERS_OF_TEN) {
            float_mantissa =
                ...
                ClingerConsts<T>::POWERS_OF_TEN_ARRAY[exp10 -
                                                      ClingerConsts<T>::EXACT_POWERS_OF_TEN];
            exp10 = ClingerConsts<T>::EXACT_POWERS_OF_TEN;
        }
        if (float_mantissa > ClingerConsts<T>::MAX_EXACT_INT) {
            return std::nullopt;
        }
        result = FPBits(float_mantissa * ClingerConsts<T>::POWERS_OF_TEN_ARRAY[exp10]);
    } else if (exp10 < 0) {
        if (-exp10 > ClingerConsts<T>::EXACT_POWERS_OF_TEN) {
            return std::nullopt;
        }
    ...
    if (round != RoundDirection::Nearest) {
        ...
        negative_result = FPBits((-float_mantissa) * ClingerConsts<T>::POWERS_OF_TEN_ARRAY[exp10]);
...
template <>
constexpr int32_t get_upper_bound<float>() {
    return 39;
}

template <>
constexpr int32_t get_upper_bound<double>() {
    return 309;
}
...
constexpr int32_t get_lower_bound() {
    using FPBits = typename fputil::FPBits<T>;
    return -((FPBits::EXP_BIAS + static_cast<int32_t>(FPBits::FRACTION_LEN + FPBits::STORAGE_LEN)) /
             3);
}

template <>
constexpr int32_t get_lower_bound<float>() {
    return -(39 + 6 + 10);
}

template <>
constexpr int32_t get_lower_bound<double>() {
    return -(309 + 15 + 20);
}
...
// and outputExp2.
template <class T>
static inline FloatConvertReturn<T> decimal_exp_to_float(ExpandedFloat<T> init_num,
    ...
    FloatConvertReturn<T>           output;
    std::optional<ExpandedFloat<T>> opt_output;
...
template <class T>
static inline FloatConvertReturn<T> binary_exp_to_float(ExpandedFloat<T> init_num, bool truncated,
    ...
    // StorageType.
    uint32_t amount_to_shift_left = std::countl_zero<StorageType>(mantissa);
    ...
    uint32_t amount_to_shift_right = FPBits::STORAGE_LEN - FPBits::FRACTION_LEN - 1;
...
static inline bool is_float_hex_start(const char* __restrict src, const char decimalPoint) {
...
// here.
template <class T>
static inline StrToNumResult<ExpandedFloat<T>> decimal_string_to_float(const char* __restrict src,
    ...
    const StorageType bitstype_max_div_by_base = std::numeric_limits<StorageType>::max() / BASE;
    while (true) {
        if (isdigit(src[index])) {
            ...
            if (mantissa < bitstype_max_div_by_base) {
                ...
                if (digit > 0) truncated = true;
                if (!after_decimal) ++exponent;
            }
    ...
    if (tolower(src[index]) == EXPONENT_MARKER) {
        ...
        if (isdigit(src[index + 1 + static_cast<size_t>(has_sign)])) {
            ...
            if (result.has_error()) output.error = result.error;
            int32_t add_to_exponent = result.value;
            index += result.parsed_len;

            // Here we do this operation as int64 to avoid overflow.
            int64_t temp_exponent =
                static_cast<int64_t>(exponent) + static_cast<int64_t>(add_to_exponent);
    ...
    if (mantissa == 0) {  // if we have a 0, then also 0 the exponent.
        ...
        auto temp    = decimal_exp_to_float<T>({mantissa, exponent}, src, truncated, round);
...
// here.
template <class T>
static inline StrToNumResult<ExpandedFloat<T>> hexadecimal_string_to_float(
    const char* __restrict src, const char DECIMAL_POINT, RoundDirection round) {
    using FPBits      = typename fputil::FPBits<T>;
    using StorageType = typename FPBits::StorageType;
...
static inline uint64_t nan_mantissa_from_ncharseq(const std::string_view ncharseq) {
    ...
    if (ncharseq.data() != nullptr && isdigit(ncharseq[0])) {
        ...
        if (!strtoint_result.has_error()) nan_mantissa = strtoint_result.value;

        if (strtoint_result.parsed_len != static_cast<ptrdiff_t>(ncharseq.size())) nan_mantissa = 0;
    }
...
template <class T>
static inline StrToNumResult<T> strtofloatingpoint(const char* __restrict src) {
    using FPBits      = typename fputil::FPBits<T>;
    using StorageType = typename FPBits::StorageType;
    using Sign        = fputil::Sign;
    ...
    if (isdigit(src[index]) || src[index] == DECIMAL_POINT) {  // regular number
        ...
        if (base == 16) {
            parse_result =
                hexadecimal_string_to_float<T>(src + index, DECIMAL_POINT, round_direction);
        } else {  // base is 10
            parse_result = decimal_string_to_float<T>(src + index, DECIMAL_POINT, round_direction);
        }
        ...
        if (tolower(src[index + 1]) == nan_string[1] && tolower(src[index + 2]) == nan_string[2]) {
            seen_digit = true;
            index += 3;
            ...
            if (src[index] == '(') {
                ...
                if (src[index] == ')') {
                    ...
                    auto nan_mantissa_result = nan_mantissa_from_ncharseq(
                        std::string_view(src + (left_paren + 1), index - left_paren - 2));
        ...
        if (tolower(src[index + 1]) == inf_string[1] && tolower(src[index + 2]) == inf_string[2]) {
            seen_digit = true;
...
template <class T>
static inline StrToNumResult<T> strtonan(const char* arg) {
    using FPBits      = typename fputil::FPBits<T>;
    using StorageType = typename FPBits::StorageType;
    ...
    ptrdiff_t index = 0;
    while (isalnum(arg[index]) || arg[index] == '_') ++index;

    if (arg[index] == '\0') {
        auto nan_mantissa_result = nan_mantissa_from_ncharseq(std::string_view(arg, index));
        nan_mantissa             = static_cast<StorageType>(nan_mantissa_result);
    }
