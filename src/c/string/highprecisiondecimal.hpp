...
class HighPrecisionDecimal {
  ...
  private:
    bool should_round_up(int32_t roundToDigit, RoundDirection round) {
        if (roundToDigit < 0 || static_cast<uint32_t>(roundToDigit) >= this->num_digits) {
            return false;
        }
    ...
    uint32_t get_num_new_digits(uint32_t lShiftAmount) {
        const char* power_of_five = LEFT_SHIFT_DIGIT_TABLE[lShiftAmount].power_of_five;
        ...
        while (power_of_five[digit_index] != 0) {
            ...
            if (this->digits[digit_index] != power_of_five[digit_index] - '0') {
                return new_digits -
                       ((this->digits[digit_index] < power_of_five[digit_index] - '0') ? 1 : 0);
            }
            ++digit_index;
        }
        return new_digits;
    ...
    void left_shift(uint32_t shiftAmount) {
        ...
        // writing.
        while (read_index >= 0) {
            accumulator += static_cast<uint64_t>(this->digits[read_index]) << shiftAmount;
    ...
    HighPrecisionDecimal(const char* __restrict numString) {
        ...
        while (isdigit(*numString) || *numString == '.') {
            if (*numString == '.') {
                ...
                ++total_digits;
                if (this->num_digits < MAX_NUM_DIGITS) {
                    this->digits[this->num_digits] = static_cast<uint8_t>(*numString - '0');
                    ++this->num_digits;
                } else if (*numString != '0') {
                    this->truncated = true;
                }
        ...
        if (!saw_dot) this->decimal_point = total_digits;
